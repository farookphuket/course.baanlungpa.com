    ...
    methods:{
        getBlog(page){
            ...
            if(page){
                ...
                this.$cookies.set("mb_old_page",url)
            }
            url = this.$cookies.get("mb_old_page")
            if(!url) url = `/api/member/blog?perpage=${this.perpage}`
            axios.get(url)
                .then(res=>{
